...
w1 = tf.Variable(tf.random_normal(shape=[3, 1], mean=0, stddev=0.05))
...
# update_w = optimizer.minimize(sum_loss)
...
update_w = w1 - lr * grad_w
w1_update = tf.placeholder(dtype=tf.float32, shape=[3, 1])
assign_op_w = w1.assign(w1_update)
...
with tf.Session() as sess:
    ...
    l_all = []
    for i in range(50):
        l, w_t = sess.run([sum_loss, update_w], feed_dict={x: X, y: Y})
        l_all.append(l)
        print(l)
        # method 3
        sess.run([assign_op_w], feed_dict={w1_update: w_t})
